...
export default function AdminGalleryPage() {
  ...
  const handleUploadComplete = (uploadedImages: any[]) => {
    ...
    toast.success(`Uploaded ${uploadedImages.length} images successfully!`)
  }
  ...
  const toggleFeatured = async (imageId: string, currentStatus: boolean) => {
    try {
      ...
      if (response.ok) {
        ...
        toast.success(`Image ${!currentStatus ? 'added to' : 'removed from'} homepage slider`)
      }
    } catch (error) {
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Upload and manage your photography images
          </p>
        </div>
